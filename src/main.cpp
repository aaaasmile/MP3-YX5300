...
const int buttonPrev = D4;  // Botton for previous song (D0 is not working, stay
                            // always LOW, D1 also with D2 and D3)
...
extern uint16_t g_maxSongs[];
extern uint8_t g_rndSongs[];
...
int get_curr_song() { return sequence.GetCurrSongIx(); }
...
String decodeMP3Answer() {
    ...
    switch (g_ans_buf[3]) {
        ...
        case 0x3D:
            decodedMP3Answer +=
                " -> Completed play num " + String(g_ans_buf[6], DEC);
        ...
        case 0x42:
            decodedMP3Answer +=
                " -> Status playing: " + String(g_ans_buf[6], DEC);
            break;
        ...
        case 0x4E:
            decodedMP3Answer +=
                " -> Folder file count: " + String(g_ans_buf[6], DEC);
            break;

        case 0x4F:
            decodedMP3Answer +=
                " -> Folder count: " + String(g_ans_buf[6], DEC);
            break;
    }
...
void loop() {
    ...
    switch (g_state) {
        case EnS_WaitForStartSeq:
            ll = micros() + g_ll_setup + 2;
            ll = ll * analogRead(0) + 5;
            ll = ll * analogRead(2) + 6 + micros();
            sequence.CreateSeq(ll, g_maxSongs[g_currFolder],
                               g_rndSongs[g_currFolder]);
#ifdef DEBUG
...
#endif
        sendCommand(CMD_PLAY_FOLDER_FILE, g_currFolder,
                    song_ix);  // start playing the song with index
        delay(100);
