...
static int8_t Send_buf[8] = {
    0};  // Buffer for Send commands.  // BETTER LOCALLY
static uint8_t ansbuf[10] = {
    0};  // Buffer for the answers.    // BETTER LOCALLY
...
void sendMP3Command(char c) {
    switch (c) {
        ...
        case '>':
            ...
            sendCommand(
                CMD_PLAYING_N);  // ask for the number of file is playing
            break;
...
String decodeMP3Answer() {
    ...
    switch (ansbuf[3]) {
        ...
        case 0x3D:
            decodedMP3Answer +=
                " -> Completed play num " + String(ansbuf[6], DEC);
            // sendCommand(CMD_NEXT_SONG);
            // sendCommand(CMD_PLAYING_N); // ask for the number of file is
            // playing
            break;
        ...
        case 0x4E:
            decodedMP3Answer +=
                " -> Folder file count: " + String(ansbuf[6], DEC);
            break;
...
void sendCommand(byte command) { sendCommand(command, 0, 0); }
